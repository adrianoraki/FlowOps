...
> | `comentarios` | **"Serviço Realizado"** — igual em todo lugar (app: seção "Descrição do Problema" com o campo rotulado assim; web: label do textarea; PDF: título do bloco). Antes de 2026-07 web/PDF ainda diziam "Descrição do Problema" (rótulo antigo, não acompanhou a troca feita só no app) — alinhado | da OS | técnico | o que o **TÉCNICO** diagnosticou/identificou |
> | `descricaoServicoRealizado` | **"Comentários"** — igual em todo lugar (app: seção assim rotulada, campo "O que foi feito pelo técnico" + ajuda "Use para registrar irregularidades encontradas, seja no chamado, na infraestrutura, etc."; web/PDF: idem, rótulo alinhado em 2026-07) | da OS | técnico | o que o **TÉCNICO** fez |
...
- `OrdemServicoForm.tsx` (admin/gestor editando a OS na web): `entrada`/`saida` usam `<input type="datetime-local">` (não mais `type="time"`) — corrigido em 2026-07 porque editar manualmente um horário ali sobrescrevia a versão com data completa (gravada pelo fluxo do técnico) por só `"HH:MM"`, perdendo a data. Carrega valores legados/ISO via `paraDataHorario` + `paraDatetimeLocal` (`packages/types`, ambas exportadas — a segunda também usada em `OrdemServicoVer.tsx` pro modal de finalização) e grava sempre como ISO completo. `formatarHora`/`formatarDataHora`/`calcularTempoTotal` continuam aceitando os dois formatos pra OS antigas já gravadas só com `"HH:MM"`.
